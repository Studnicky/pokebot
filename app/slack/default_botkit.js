var default_botkit = {
	...
	events: function(controller, bot){
		...
		controller.on('bot_channel_join', function(bot, data){
			console.log('The bot has joined a channel');
			console.log(data);
		});

		controller.on('user_channel_join', function(bot, data){
			console.log('A user has joined a channel');
			console.log(data);
		});

		controller.on('bot_group_join', function(bot, data){
			console.log('The bot has joined a group');
			console.log(data);
		});

		controller.on('user_group_join', function(bot, data){
			console.log('A user has joined a group');
			console.log(data);
		});

		controller.on('direct_message', function(bot, data){
			console.log('The bot received a direct message from a user');
			console.log(data);
		});

		controller.on('direct_mention', function(bot, data){
			console.log('The bot was addressed directly in a channel');
			console.log(data);
		});

		controller.on('mention', function(bot, data){
			console.log('The bot was mentioned by someone in a message');
			console.log(data);
		});

		controller.on('ambient', function(bot, data){
